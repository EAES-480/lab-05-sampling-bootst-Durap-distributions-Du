...
> GM GRADE: 9.75/10 - excellent work!
...
```{r pop_params, echo=TRUE, eval=TRUE}
...
> The variable “le” has the largest population standard deviation (approximately 80.6), indicating that it exhibits the greatest variability over time among the selected variables. 
...
```{r one_sample_relerr, echo=TRUE, eval=TRUE}
...
> The variable “fc” had the largest relative error (316%). This likely occurred because the population mean of fc is close to zero. When the true mean is small, even modest differences between the sample mean and the population mean can produce very large relative errors.
...
```{r relerr_by_n, echo=TRUE, eval=TRUE}
set.seed(480)
...
```{r summarize_plot_relerr, echo=TRUE, eval=TRUE}
# GOAL: Summarize and plot the relationship between n and relative error.
# TODO: write your own code.

summary_relerr <- sim_relerr %>% group_by(n) %>% summarise( median_relerr = median(rel_error_pct), p90_relerr = quantile(rel_error_pct, 0.9))
summary_relerr
...
> Accuracy improves when increasing sample size from very small values (e.g., 25 to 100), but gains become smaller at larger sample sizes (e.g., 300 to 1000). As n becomes larger, the sample mean is already fairly stable, so adding more observations yields smaller incremental reductions in error (diminishing returns). 
...
```{r se_compare, echo=TRUE, eval=TRUE}
set.seed(480)
...
```{r plot_sampling_dist, echo=TRUE, eval=TRUE}
...
> The `se_theory`(0.8495689	) and `se_simulated` (0.8731462)  were very close. Increasing n_samp reduces the standard error. The variability in the sample mean decreases as sample size increases. Increasing reps, however, does not change the true SE; it only smooths our simulation estimate of the sampling distribution.

> GM: Great answer! minor rephrase: "n_samp reduces the standard error estimated from the spread of the bootstrap distribution"
...
```{r bootstrap_mean, echo=TRUE, eval=TRUE}
set.seed(480)
...
> The bootstrap distribution approximates the sampling distribution of the mean using only one observed sample. Instead of repeatedly sampling from the true population (which is usually unknown in real-world EAES studies), we resample with replacement from our observed data. Bootstrap is especially useful in environmental science where field campaigns are expensive and we often only have one limited dataset.
...
```{r part2_prep, echo=TRUE, eval=TRUE}
...
```{r ex1_random_bootstrap, echo=TRUE, eval=TRUE}
set.seed(480)
...
```{r ex1_plot_random_boot, echo=TRUE, eval=TRUE}
...
> The bootstrap distribution is centered around the sample mean (dashed line), and it is relatively close to the population mean (solid line), although there is a small difference between the two. The bootstrap SD represents the estimated standard error of the sample mean, meaning it reflects how much the mean would vary if we repeatedly sampled under the same conditions. 
...
```{r ex2_summer_sample, echo=TRUE, eval=TRUE}
...
```{r ex2_summer_bootstrap, echo=TRUE, eval=TRUE}
set.seed(480)
...
```{r ex2_plot_summer_boot, echo=TRUE, eval=TRUE}
...
> The bootstrap distribution appears narrow and well-defined, which suggests high precision. But, it is clearly centered far from the population mean. This indicates that although the estimate looks confident, it is not accurate. The summer-only sampling introduces seasonal bias because it does not represent the full annual variability of the sample data. The bootstrap correctly measures uncertainty around the biased sample mean, but it cannot fix a biased sampling design.
...
```{r ex3_stratified_sample, echo=TRUE, eval=TRUE}
set.seed(480)
...
```{r ex3_strat_bootstrap, echo=TRUE, eval=TRUE}
set.seed(480)
...
```{r ex3_plot_strat_boot, echo=TRUE, eval=TRUE}
...
> The stratified sampling mean is closest to the pop annual mean. The random and stratified samples both produce bootstrap distributions that are centered close to the population mean, while the summer-only sample is clearly centered far from it. All three distributions appear relatively narrow, indicating high precision. However, only the random and stratified designs provide accurate estimates of the annual mean. The summer-only sample is precise but biased due to seasonal sampling. This comparison shows that bootstrap measures precision around the observed sample mean, but it cannot correct for poor study design or sampling bias.
...
```{r ex1_boot_ci_mean, echo=TRUE, eval=TRUE}
...
The 95% bootstrap confidence interval for the mean SWC is approximately 34.78 to 36.93. This suggests that the sample estimate is reasonably accurate and that the bootstrap method provides a realistic estimate of the uncertainty around the sample mean.
...
```{r ex2_plot_ci, echo=TRUE, eval=TRUE}
...
> The two solid red vertical lines mark the 2.5th and 97.5th percentiles of the bootstrap distribution, which form a 95% bootstrap confidence interval for the mean. This interval represents the range of mean values that are consistent with the observed sample under bootstrap resampling.
...
```{r ex3_fit_model, echo=TRUE, eval=TRUE}
df_sample<-df %>% slice_sample(n=1000,replace=F)
response_var <- "swc"
predictor_var <- "ta"
...
model <- lm(reformulate(predictor_var, response_var), data = df_mod)
coef(model)
```

### Bootstrap the regression

```{r ex3_bootstrap_lm, echo=TRUE, eval=TRUE}
set.seed(480)
...
boot_coef <- replicate(B, {
  ...
  samp_rows <- df %>% slice_sample(n = nrow(df_sample), replace = TRUE)
...
colnames(boot_coef) <- c("intercept", "slope")
head(boot_coef)
```

### Compute confidence intervals

```{r ex3_ci_params, echo=TRUE, eval=TRUE}
...
> No, this interval does not include zero. The 95% bootstrap confidence interval for the slope is approximately −0.475 to −0.392. It suggests that there is a consistent negative relationship between air temperature (ta) and soil water content (swc) in the data. In plain language, higher temperatures tend to be associated with lower soil water content. This result is consistent with the expectation that warmer conditions increase evaporation and soil moisture loss.

> GM: Good answer overall. -0.25 We'd say though that "the relationship between the predictor and response variable is statistically significant".
